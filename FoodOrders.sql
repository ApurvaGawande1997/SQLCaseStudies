...
--a)Top 3 outlets by cuisine type without using limit or top
with cte as (
...
--b) Find the daily new customer count from the launch date (everyday how many new customers are we acquiring)
with cte as(
...
--c) Count all the users who were acquired in Jan 2025 and only placed order in Jan and did not place any order other than that
select * from orders

select Customer_code,COUNT(DISTINCT Customer_code) as no_of_orders
from orders
where MONTH(Placed_at)=1 and YEAR(Placed_at)='2025' and Customer_code NOT IN ( select Customer_code from orders where MONTH(Placed_at)>1 )
group by Customer_code
having COUNT(*)=1 
 
--d)List all the customers with no order in the last 7 days but were acquired one month ago with their first order on promo code
with cte as (
select Customer_code,MIN(Placed_at) as first_order_date,MAX(Placed_at) as latest_order_date
from orders 
group by Customer_code)
select c.*,o.Promo_code_Name as first_order_promo
from cte c inner join orders o on c.Customer_code=o.Customer_code AND c.first_order_date = o.Placed_at
where latest_order_date < DATEADD(DAY,-7,GETDATE()) and
first_order_date < DATEADD(MONTH,-1,GETDATE()) and o.Promo_code_Name IS NOT NULL

--e) Growth team is planning to create a trigger that will target customers after their every third order with
-- personalized communication and they have asked you to create a query for this
with third_order_trigger as (
select *,
ROW_NUMBER() over (partition by Customer_code order by Placed_at) as order_number
from orders
)
select *
from third_order_trigger
where order_number%3=0 and cast(Placed_at as date)=cast(GETDATE() as date)

--f) List customers who placed more than 1 order and all of their orders are on promo code
select Customer_code,COUNT(*) as no_of_orders,COUNT(DISTINCT Promo_code_Name) as Promo_count
from orders
group by Customer_code
having COUNT(distinct order_id)>1 and COUNT(DISTINCT Promo_code_Name)=COUNT(distinct order_id)

--g) What percent of customers are acquired in jan 2025 organically (order placed withput any promo code)

select SUM(percentage_customers) as Total_Percentage from (
select Customer_code,Promo_code_Name,(COUNT(DISTINCT Customer_code)*1.0/(select COUNT(DISTINCT Customer_code) from orders))*100 as percentage_customers
from orders
where MONTH(Placed_at)=1 and YEAR(Placed_at)=2025 and Promo_code_Name IS NULL
group by Customer_code,Promo_code_Name
) A
